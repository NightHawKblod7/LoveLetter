...
import javax.swing.*;
public class LovePanel extends JPanel implements MouseListener {
	private BufferedImage one, two, three, four, five, six, seven, eight, nine, ten, eleven, twelve, thirteen, fourteen, fifteen,
	sixteen, seventeen, eighteen, nineteen, twenty, twentyOne, twentyTwo, choice, choice2, choice3;
	private ArrayList<BufferedImage> list, list2;
	private boolean choose;
	private int turn=0;
	public LovePanel() {
		try {
			one  = ImageIO.read(new File("spy.png"));
			two  = ImageIO.read(new File("guard.png"));
			three  = ImageIO.read(new File("baron.png"));
			four  = ImageIO.read(new File("priest.png"));
			five  = ImageIO.read(new File("handmaid.png"));
			six  = ImageIO.read(new File("prince.png"));
			seven  = ImageIO.read(new File("chancellor.png"));
			eight  = ImageIO.read(new File("king.png"));
			nine  = ImageIO.read(new File("countess.png"));
			ten  = ImageIO.read(new File("LoveLetterImagePrincess.png"));
			eleven  = ImageIO.read(new File("back.png"));
		}
		catch(Exception e) {
	...
	public void paint(Graphics g) {
		g.drawImage(eleven, 1000, 50, getWidth()/12, getHeight()/5, null);
		if(turn==0) {
			g.drawImage(choice,  150,  100,  getWidth()/12,  getHeight()/5, null); 
		}
		turn++;
		g.drawImage(choice3, 1500, 50, getWidth()/12, getHeight()/5, null);
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(e.getButton()== e.BUTTON1) {
			if(!choose && x >= 1000 && y >= 50 &&
					x <= getWidth()/12+1000 && y <= getHeight()/5+50)
				choice = list.get(0);
		}
		repaint();
		list.remove(0);
